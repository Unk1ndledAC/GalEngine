...
export type SidebarView = 'explorer' | 'plugins' | 'llm' | 'debug';
...
export const App: React.FC = () => {
  ...
  const vfs: VFS | undefined = galApi?.fs ? {
    ...
    readBinaryFile: (p: string) => galApi.fs.readBinaryFile(p),
    writeTextFile: (p: string, c: string) => galApi.fs.writeTextFile(p, c),
    writeBinaryFile: (p: string, d: Uint8Array) => galApi.fs.writeBinaryFile(p, d),
